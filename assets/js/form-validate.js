...
function valid_datas(f) {
    if (f.name.value == "") {
        ...
        jQuery.ajax({
            url: "mail.php",
            type: "post",
            data: jQuery("form#chefretard-contact").serialize(),
            complete: function (data) {
                jQuery("#form_status").html(data.responseText);
                jQuery("#chefretard-contact")
                    .find("input,textarea")
                    .attr({ value: "" });
                jQuery("#chefretard-contact").css({ opacity: 1 });
                jQuery("#chefretard-contact").remove();
            },
        });
        jQuery("#form_status").html(
            '<span class="loading">Sending your message...</span>'
        );
        jQuery("#chefretard-contact").animate({ opacity: 0.3 });
        jQuery("#chefretard-contact")
...
function notice(f) {
    jQuery("#chefretard-contact").find("input,textarea").css("border", "none");
